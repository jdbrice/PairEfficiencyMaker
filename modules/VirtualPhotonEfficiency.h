#ifndef VIRTUAL_PHOTON_EFFICIENCY_H
#define VIRTUAL_PHOTON_EFFICIENCY_H
...
class VirtualPhotonEfficiency : public HistoAnalyzer {
protected:
...
public:
	...
	virtual void initialize(){
		...
		pTDistribution = shared_ptr<TF1>( new TF1( "gamma_pT", "1*(0.0 < x < 5.0)", 0, 5 ) );
		pTDistribution->SetRange( 0, 5 ); // TODO: make configurable
		pTDistribution->SetNpx(500);


		massDistribution = shared_ptr<TF1>( new TF1( "gamma_mass", "1*(0.210 < x < 5.0)", 0, 5 ) );

		// Set the BreitWigner to use the width and mass of this plc
		// massDistribution->SetParameter( 0, jpsiW );
		// massDistribution->SetParameter( 1, jpsiM );

		massDistribution->SetRange( 0.210, 2 ); // TODO: make configurable
		...
		ceres = shared_ptr<TF1>( new TF1( "jpsi_y", "pow(cosh(3*x/4./sqrt(log([0]/2./[1]))/(1-pow(x,2)/2./[0]*[2])),-2.)" ) );
		// p0 = sqrt_s
		// p1 = nucleon mass
		// p2 = hadron mass
		ceres->SetParameters( 200, 0.938, 1.5);
		ceres->SetRange( -1, 1 );
		...
		double eta = rapidityToEta( ceres->GetRandom(), 1.0, 3.0 );
		// should be flat
	...
	virtual void makeWithParentKinematics(){
		...
		while ( nPass < N ){
			...
			double m   = massDistribution->GetRandom();
			double pt  = pTDistribution->GetRandom();
			...
			book->fill( "hmtdr", rclv.M(), rclv.Pt(), eff );
